...
cross.each {|dict|
  ...
  # For 3 degrees of freedom, 0.05-probability threshold is 7.81
  if inst.chi_sq() > 7.81   
...
puts "###################"
puts
puts "2- My seed-stock object represents the entire database. All entries in"
puts "   the tsv file have been read correctly using load_from_file method."
puts; db.database.each {|k, v| puts "{:#{k}=> #{v}}"}
puts
puts "   Also, the object accesses individual rows based on their ID and has "
puts "   a write_database method (writes a tsv file with updated database)."
...
while test2 == "y"
  ...
  puts
  puts "   This is the information about #{stock} ID stored in the database:"
  puts
  puts db.get_seed_stock(stock)
  
  puts
  puts "   And below you see the tsv file generated with the updated database."s
  puts
  system("cat #{ARGV[3]}")
